...
import TableComponent from '../../common/invoicereport';
...
const App = () => {
    ...
    const handleDownload = () => {
        ...
        const headers = [
            ["Bu", "Bu SL", "Bu Vertical", "Customer", "Project", "Total"].concat(months.flatMap(month => [month, "", "", "Total"])),
            ["", "", "", "", "", ""].concat(months.flatMap(() => ["Invoiced", "Recognised", "Advance", "Total"]))
        ];
        ...
        const rows = transformedData.map(row => {
            ...
            months.forEach(month => {
                result.push(row.months[month]?.invoiced || '-');
                result.push(row.months[month]?.recognised || '-');
                result.push(row.months[month]?.advance || '-');
                result.push((row.months[month]?.invoiced || 0) +
                            (row.months[month]?.recognised || 0) +
                            (row.months[month]?.advance || 0));
            });
