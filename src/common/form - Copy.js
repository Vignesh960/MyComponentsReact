...
function DynamicForm({ formConfig, onSubmit }) {
    ...
    const [parentProjectBudget, setParentProjectBudget] = useState(1000000);

    useEffect(() => {
        resetBudget();
    }, [parentProject, parentProjectBudget]);


    const resetBudget = () => {
        setFormValues(prevValues => ({ ...prevValues, budget: '' }));
    };
        ...
        <form onSubmit={handleSubmit} className="container">
            ...
            <>
                ...
                {errors[field.name] && <div className="invalid-feedback">{errors[field.name]}</div>}
                        ...
                        {field.type === 'text' && field.name !== 'budget' && (
                            <>
                                <input
                                    type="text"
                                    className={`form-control form-control-sm ${errors[field.name] && 'is-invalid'}`}
                        ...
                        {field.type === 'text' && field.name === 'budget' && (
                            <>
                                <input
                                    ...
                                    className={`form-control form-control-sm ${errors[field.name] && 'is-invalid'}`}
                                    name={field.name}
                                    value={formValues[field.name] !== '' ? formValues[field.name] : field.initialValue}
                                    onChange={(e) => handleChange(field.name, e.target.value)}
                                    disabled={field.isDisabled} // Add disabled attribute based on isDisabled prop
                                />
                                {errors[field.name] && <div className="invalid-feedback">{errors[field.name]}</div>}
